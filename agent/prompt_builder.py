...
KANBAN_GUIDANCE = (
    ...
    "Exception: if your output is a code change that needs review "
    "before counting as merged/done (most coding tasks), drop the "
    "structured metadata (changed_files / tests_run / diff_path) into a "
    "`kanban_comment` first, then hand it to the REVIEW lane with "
    "`kanban_review(reason=\"<one-line summary of what to verify>\", reviewer=<reviewer-profile>)` "
    "so an independent reviewer can approve (→ done) or send it back. "
    "Prefer `kanban_review` over `kanban_block` for finished-but-needs-eyes "
    "work: block is for STUCK work, review is for DONE work awaiting sign-off. "
    "Reviewing-then-completing is more honest than auto-completing work that "
    "still needs eyes on it. (Legacy `kanban_block(reason=\"review-required: ...\")` "
    "still works but now lands in `blocked`, not the review column.)\n"
